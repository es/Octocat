var im = require('imagemagick'), fs = require('fs'), http = require('http'), url = require('url'), async = require('async');
...
var server = http.createServer(function(req, res){
  ...
  if (!query.id) {
  ...
  else if (Number(query.id) === NaN || query.id > octoArr.length || query.id === 0) {
  	res.writeHead(404, {'Content-Type': 'text/plain'});
  	res.end("Bad ID value!");
  }
  else  {
  	var octocat = octoArr[Number(query.id)-1];
    fs.readFile('img/' + octocat.img, function(err, img) {
    	if (err) {
    		...
    		async.series([
			    function(callback){
			    	console.log('query.x && query.y:',query.x && query.y);
			    	console.log('query.x:',query.x);
			    	console.log('query.y:',query.y);
			    	if (query.x && query.y) {
			    		im.resize({
						  srcData: img,
						  width: query.x,
